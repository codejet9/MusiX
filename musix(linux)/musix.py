...
def musix():
    ...
    def streamSong(songURL):
        ...
        def events():
            ...
            combinations = {
                frozenset([Key.space]): pausePlayMusic,
            }
            ...
            def on_press(key):
                ...
                for combination in combinations:  # Loop through each combination
                    if is_combination_pressed(combination):  # Check if all keys in the combination are pressed
                        combinations[combination]()  # If so, execute the function
            #
        ...
        while str(player.get_state()) in states:
            ...
            if str(player.get_state())=="State.Playing" and playing==False:
                ...
                playing=True; pause=False; loading=False;

            if str(player.get_state())=="State.Paused" and pause==False:
                print("Status: Paused       ",end='\r')
                pause=True; playing=False; loading=False;
        #

        print("")
        print("Status: Finish")

        stop(); # remove all hotkeys
